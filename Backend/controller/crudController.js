...
const postUser = async (req, res) => {
  const { name, phone, adminid, state, district, area,group} = req.body;
  // console.log(req.body);
  try {
    ...
    if (existingUser) {
      ...
      // console.log("2");
      const feature = await User.create({ name, phone, state, district, area, adminid,group })
...
const bulkupload = async (req, res) => {
  try {
    ...
    for (const record of data) {
      const { name, phone, state, district, area,group } = record;
      ...
      if (isValidPhone) {
        ...
        // console.log(existingUser);
        if (!existingUser) {
          const newUser = new User({ name, phone, adminid, state, district, area ,group});
          await newUser.save();
        }
      } else {
...
const deleteUser=async(req,res)=>{
  try {
    // Find all user documents with the specified adminid
    // Delete all found documents
    const result = await User.deleteMany({ adminid: req.query.adminid });
    // Respond with the deleted documents if needed
